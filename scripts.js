...
ScrollReveal().reveal('.home-content h3, .home-content p, .about-content', {
  ...
  function applyMarginForDesktop() {
    const aboutContent = document.querySelector('.about-content');
    if (window.matchMedia('(min-width: 768px)').matches) {
      aboutContent.style.marginTop = '-20rem'; // Desktop
    } else {
      aboutContent.style.marginTop = '0rem'; // Mobile
    }
  }

  // Run on load
  applyMarginForDesktop();

  // Run on resize
  window.addEventListener('resize', applyMarginForDesktop);